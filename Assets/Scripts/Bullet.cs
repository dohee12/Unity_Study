...
// Goal : 앞으로(Z축) 날아가서, 무언가에 충돌하겠지.
// - 방향(direction) : z축(forward)
// - 속력(speed)
// - 충돌 : 실체(collider) + 물리연산적용(Rigidbody)
//   ㄴ A.어딘가에 부딪히면 파괴(자기자신을 파괴)
//   ㄴ B.어디에도 부딪히지 않는 경우, 일정 시간이 지나면 파괴
...
public class Bullet : MonoBehaviour
{
    ...
    // Unity 실행 시 가장 먼저 실행되는 Lifecycle함수(onEnable,Start보다 먼저)
    void Awake()
    {
        // bullet(나자신)에게 붙어있는 Rb 값을 할당(=초기화)
        rb = GetComponent<Rigidbody>();
    }
    // GameObject가 활성활될 때마다 실행되는 Lifecycle함수
    void OnEnable()
    {
    ...
    void Update()
    {
        ...
        // 1.Timer가 일정시간(lifeTime)이 지난 경우
        if(timer > lifeTime){
            // Goal. 총알(=자기 자신) 파괴
            DeactiveMySelf();
        }
    ...
    void DeactiveMySelf(){
        ...
        //player.transform.GetChild(2).GetComponent<Gun>();
    ...
    void OnCollisionEnter(Collision other)
    {
        ...
        bool compareLayer = other.gameObject.layer.Equals("Enemy");
